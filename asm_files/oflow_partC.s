 ...
 *  check on the value of two local variables. The first is the target which is check for 
 *  a particular value: "pass" (non-zero terminated). The other is a canary value that is 
 *  to ensure the user does not overwrite further in the stack than necessary. The canary 
 *  value is between the vulnerable buffer and the target value. Therefore the user must 
 *  preserve the value of the canary while overwritting the target with the correct value.
 *  If the canary value is unaltered and the target value is correct the program will print 
 *  a secret key.
 * Input: 
...
main:
  ...
  mov r4, sp        // Space for target
  sub sp, sp, canary_size
  mov r5, sp        // Space for canary
  ...
  // Test target == "pass"
  ldr r1, [r4]
  ldr r2, =#0x73736170    //ssap
  cmp r1, r2
  addeq r8, r8, #1
  blne target_fail
  
  // Test canary == "hacker"
  ldr r1, =#0x6B636168  // kcah
  ldr r2, [r5]
  eor r3, r1, r2
  ldr r1, =#0x7265      // "re"
  ldr r2, [r5, #4]
...
.balign 4
  ...
  _success_str: .asciz "Congrats! You overwrote the target while perserving the canary!\n"
  .set _success_str_len, .-_success_str
  _key_str:     .asciz "The key is: \"hack the planet\"\n"
